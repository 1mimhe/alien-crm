...
۲. **اجرای همگام‌ساز خودکار از داخل ایران (پیشنهادی)**:
اگر به دلیل محدودیت دیتاسنتر داخلی و فیلترشکن با خطای ۵۲۲ مواجه می‌شوید، اسکریپت آماده زیر را اجرا کنید:
```bash
node sync-local.js
```
این اسکریپت:
- فیلترشکن/تونل شما را خودکار بای‌پس کرده و از شبکه داخل کشور متصل می‌شود.
- در صورت انقضای توکن، با یوزرنیم و پسورد لاگین مجدد می‌کند.
- اطلاعات کارتابل را دریافت کرده، به کلودفلر ورکر تحویل می‌دهد تا در گوگل شیت درج شود.
- هر ۱۰ دقیقه یک‌بار به طور خودکار اجرا و به‌روزرسانی می‌شود.
